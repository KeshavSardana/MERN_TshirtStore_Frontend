...
const AddCategory = () => {
  ...
  const preloadTheCategory = () => {
    getCategory(name._id).then((data) => {
      if (data.error) {
  ...
  const onSubmit = (event) => {
    ...
    updateCategory(name._id, user._id, token, { name })
      .then((data) => {
        if (data.error) {
          setError(data.error);
          setSuccess(false);
  ...
  const successMessage = () => {
    if (success) {
      return (
        <h4 className="text-success my-3"> Category Updated successfully</h4>
      );
    }
  };
  ...
  const errorMessage = () => {
    if (error) {
      return <h4 className="text-warning my-3">Failed to update Category</h4>;
    }
  };
